...
{
	auto reply = buses.insert(std::pair<int, Bus>(bus.get_number(), bus));
...
{
	return buses.find(number) != buses.end();
}

std::string transport_manager::get_bus_as_str(int number) const
{
	auto search_index = buses.find(number);
...
{
	auto search_index = buses.find(b_number);
...
{
	auto search_index = buses.find(bus_number);
	std::string result{ "Bus number " };
	result += std::to_string((search_index->second).get_number());
	result += "\nDriver name ";
	result += (search_index->second).get_driver_name();
